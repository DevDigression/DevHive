...
router.get("/test", (req, res) => {
  res.json({ message: "Profile tested successfully!" });
});
...
router.get("/all", (req, res) => {
  ...
  Profile.find()
    ...
    .then(profiles => {
      if (!profiles) {
        errors.noprofile = "No user profiles currently exist.";
        return res.status(404).json(errors);
      }
      res.json(profiles);
    })
    .catch(err =>
      res.status(404).json({ profile: "No user profiles currently exist." })
    );
});
...
router.get("/user/:user_id", (req, res) => {
  ...
  Profile.findOne({ user: req.params.user_id })
    ...
    .catch(err => res.json(404).json({ profile: "User profile note found" }));
});
